...
/* Table de correspondance caractère <-> code morse, stockée en Flash (données constantes) */
...
/* Nombre d'entrées dans la table, calculé automatiquement à la compilation */
#define MORSE_TABLE_COUNT (sizeof(MORSE_TABLE_DATA) / sizeof(MORSE_TABLE_DATA[0]))

/* Vérifie si deux codes morse sont identiques (même longueur et mêmes symboles) */
...
/* Initialise la table en la faisant pointer vers les données constantes */
...
/* Cherche le code morse d'un caractère en parcourant la table séquentiellement */
MorseStatus morse_lookup_char(const MorseTable *table, char c, MorseCode *out)
{
    if (table == NULL || out == NULL) return MORSE_ERR_NULL;

    char upper = (char)toupper((unsigned char)c); /* Convertit en majuscule pour uniformiser */
...
/* Cherche le caractère correspondant à un code morse donné */
